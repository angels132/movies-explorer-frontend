...
function Register(props) {
  ...
  const handleSubmit = (e) => {
    ...
    if (isNameValid(name) && isEmailValid(email) && isPasswordValid(password)) {
      ...
      setPasswordError(
        isPasswordValid(password) ? '' : 'Минимальная длина пароля 8 символов');
    }
  };
    ...
    <main className="register" id="signup">
      ...
      >
        ...
        <input
          ...
          value={name}
        />
        <span className={`register__error ${isNameValid(name) ?
          '' :
          'register__error_active'}`}>{nameError}</span>
        ...
        <input
          ...
          value={email}
        />
        <span className={`register__error ${isEmailValid(email) ?
          '' :
          'register__error_active'}`}>{emailError}</span>
